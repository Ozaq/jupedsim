...
#define fail(message)                                                                              \
    ...
    for(int i = 0; i < P; i++) {
        ...
        for(int j = 0; j < n; j++) {
            ...
            if(vertex_index >= V) {
                std::cerr << "Got a vertex index of " << vertex_index << std::endl;
...
void Mesh::precalc_point_location()
{
    ...
    for(auto& pair : slabs) {
        std::sort(pair.second.begin(), pair.second.end(), [&](const int& a, const int& b) -> bool {
...
PolyContainment Mesh::poly_contains_point(int poly, Point& p)
{
    ...
    for(int i = 0; i < (int) poly_ref.vertices.size(); i++) {
        ...
        if(std::abs(cur_a) < EPSILON) {
            ...
            return {PolyContainment::ON_EDGE, poly_ref.polygons[i], point_index, last_index};
        }
...
{
    if(p.x < min_x - EPSILON || p.x > max_x + EPSILON || p.y < min_y - EPSILON ||
       p.y > max_y + EPSILON) {
    ...
    const auto close_it = std::lower_bound(
        polys.begin(), polys.end(), p.y, [&](const int& poly_index, const double& y_coord) -> bool {
            // Sorts based on the midpoints.
            // If tied, sort based on width of poly.
            const Polygon& poly = mesh_polygons[poly_index];
            return poly.min_y + poly.max_y < y_coord * 2;
        });
    const int close_index = close_it - polys.begin() - (close_it == polys.end());
    ...
    while(i >= 0 && i < ps) {
        ...
        switch(result.type) {
            ...
            case PolyContainment::ON_EDGE:
                ...
                return {
                    (result.adjacent_poly == -1 ? PointLocation::ON_MESH_BORDER :
            ...
            case PolyContainment::ON_VERTEX:
                // This one lies on a corner.
                {
                    ...
                    if(v.is_corner) {
                        if(v.is_ambig) {
                            return {
                                PointLocation::ON_CORNER_VERTEX_AMBIG, -1, -1, result.vertex1, -1};
                        } else {
                            return {
                                PointLocation::ON_CORNER_VERTEX_UNAMBIG,
                                polygon,
                                -1,
                                result.vertex1,
                                -1};
                        ...
                        return {
                            PointLocation::ON_NON_CORNER_VERTEX, polygon, -1, result.vertex1, -1};
                    }
                }
...
{
    outfile << "mesh with " << mesh_vertices.size() << " vertices, " << mesh_polygons.size()
            << " polygons" << std::endl;
