...
struct PolyContainment {
    ...
    friend std::ostream& operator<<(std::ostream& stream, const PolyContainment& pc)
    {
        switch(pc.type) {
            ...
            case PolyContainment::ON_EDGE:
                return stream << "ON_EDGE (poly " << pc.adjacent_poly << ", vertices " << pc.vertex1
                              << ", " << pc.vertex2 << ")";
...
struct PointLocation {
    ...
    friend std::ostream& operator<<(std::ostream& stream, const PointLocation& pl)
    {
        switch(pl.type) {
            ...
            case PointLocation::ON_MESH_BORDER:
                return stream << "ON_MESH_BORDER (poly " << pl.poly1 << ", vertices " << pl.vertex1
                              << ", " << pl.vertex2 << ")";

            case PointLocation::ON_EDGE:
                return stream << "ON_EDGE (polys " << pl.poly1 << ", " << pl.poly2 << ", vertices "
                              << pl.vertex1 << ", " << pl.vertex2 << ")";

            case PointLocation::ON_CORNER_VERTEX_AMBIG:
                return stream << "ON_CORNER_VERTEX_AMBIG (" << pl.vertex1 << ", poly? " << pl.poly1
                              << ")";

            case PointLocation::ON_CORNER_VERTEX_UNAMBIG:
                return stream << "ON_CORNER_VERTEX_UNAMBIG (" << pl.vertex1 << ", poly " << pl.poly1
                              << ")";

            case PointLocation::ON_NON_CORNER_VERTEX:
                return stream << "ON_NON_CORNER_VERTEX (" << pl.vertex1 << ", poly " << pl.poly1
                              << ")";
    ...
    bool operator==(const PointLocation& other) const
    {
        ...
        switch(type) {
            ...
            case PointLocation::ON_EDGE:
                if(poly1 == other.poly1 && poly2 == other.poly2 && vertex1 == other.vertex1 &&
                   vertex2 == other.vertex2) {
                    return true;
                }
                if(poly1 == other.poly2 && poly2 == other.poly1 && vertex1 == other.vertex2 &&
                   vertex2 == other.vertex1) {
                    return true;
                }
                return false;
    ...
    bool operator!=(const PointLocation& other) const { return !((*this) == other); }
};
