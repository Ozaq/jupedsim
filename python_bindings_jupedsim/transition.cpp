...
#include "transition_description_wrapper.hpp"
#include <Journey.hpp>
...
{
    py::class_<TransitionDescriptionWrapper>(m, "Transition")
        .def_static(
            "create_none_transition",
            []() -> TransitionDescriptionWrapper { return {NonTransitionDescription()}; })
        .def_static(
            "create_fixed_transition",
            [](BaseStage::ID stageId) -> TransitionDescriptionWrapper {
        ...
        .def_static(
            ...
            [](const std::vector<std::tuple<BaseStage::ID, uint64_t>>& stageWeights)
                -> TransitionDescriptionWrapper {
                return {RoundRobinTransitionDescription(stageWeights)};
            })
        .def_static(
            "create_least_targeted_transition",
            [](const std::vector<BaseStage::ID>& stages) -> TransitionDescriptionWrapper {
